...
contract('Delegable', async (accounts: string[]) => {
  ...
  it('does not allow revoking delegations that do not exist', async () => {
    await expectRevert(
      delegable.revokeDelegate(user, mintSignature, { from: owner }),
      'Delegable: not-delegated'
    )
  })

  it('adds delegates', async () => {
    await delegable.addDelegate(user, mintSignature, 1000000, { from: owner })
    assert.equal(await delegable.delegates(owner, user, mintSignature), 1000000)
  })
  ...
  describe('with setup delegates', async () => {
    ...
    it('does not allow adding the same delegation twice', async () => {
      await expectRevert(
        delegable.addDelegate(user, mintSignature, 1000000, { from: owner }),
        'Delegable: already-delegated'
      )
    })

    it('changes delegates', async () => {
      await delegable.addDelegate(user, mintSignature, 1000001, { from: owner })
      assert.equal(await delegable.delegates(owner, user, mintSignature), 1000001)
    })

    it('revokes delegates', async () => {
      await delegable.revokeDelegate(user, mintSignature, { from: owner })
      assert.equal(await delegable.delegates(owner, user, mintSignature), 0)
    })
  })
})
